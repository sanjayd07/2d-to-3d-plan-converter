...
def main(argv):
    ...
    # Debug path values
    print(f"DEBUG: program_path = {program_path}")
    print(f"DEBUG: target = {target}")
    
    # If target is just a filename, construct full path to Target directory
    if not os.path.dirname(target):
        # Target is just a filename, construct full path
        output_filepath = os.path.join(program_path, "Target", target)
    else:
        # Target already has directory info, use as is
        output_filepath = os.path.join(program_path, target)
    
    print(f"DEBUG: output_filepath = {output_filepath}")
